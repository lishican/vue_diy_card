var defualts = {
  ...
  position: "",
  buttonImgSrc: "",
  htmls: '<audio autoplay loop style="width:0px;"></audio>'
};
var Player = function(options) {
  var settings = Object.assign({}, defualts, options); //缺省值合并
  var audioDom = settings.el
    ? document.getElementById(settings.el)
    : document.body; //获得用户传入的节点
  ...
  var audioFn = {
    play: function(url) {
      ...
      audioBox.style.cssText +=
        ";animation: " + settings.animaClass + " .8s linear infinite;";
    },
    stop: function() {
      audioTag.pause();
      audioBox.style.backgroundImage = audioBox.style.backgroundImage;
  ...
  if (settings.audioUrl) {
    ...
    console.error("无音乐资源启动失败，请添加音乐资源 audioUrl");
    return;
  }

  var _device = /Android|iPhone|iPad|iPod|BlackBerry|webOS|Windows Phone|SymbianOS|IEMobile|Opera Mini/i.test(
    navigator.userAgent
  );
